...
export async function fetchApiEndpoints(): Promise<{ version: string; service: string; status: string; endpoints: any[] }> {
  return {
    version: "3.0.0",
    service: "Debate & Win API (FastAPI + Groq)",
    status: "healthy",
    endpoints: [
      {
        path: "/receipts/analyze",
        method: "POST",
        category: "Forensic Autopsy",
        description: "Forensic analysis of a pasted conversation transcript.",
        requestBody: { text_content: "string" },
        responseSample: { case_id: "...", title: "...", subtextScore: 92, frameLossPct: 78, nodes: [] },
      },
      {
        path: "/receipts/upload",
        method: "POST",
        category: "Forensic Autopsy",
        description: "Forensic analysis of an uploaded screenshot (multipart image).",
        responseSample: { case_id: "...", title: "...", subtextScore: 92, frameLossPct: 78, nodes: [] },
      },
      {
        path: "/fumble/analyze",
        method: "POST",
        category: "Draft Detonator",
        description: "Evaluate a draft message for risk before sending.",
        requestBody: { draft: "string", context: "string (optional)" },
        responseSample: { threatLevel: 4, fumblePct: 82, survivalChance: 18, safeCountermeasures: [] },
      },
      {
        path: "/sparring/start",
        method: "POST",
        category: "Sparring Arena",
        description: "Start a live sparring session against an AI opponent.",
        requestBody: { topic: "string", persona: "string", aggressiveness: "number" },
        responseSample: { session_id: "...", userFramePct: 68, botFramePct: 32 },
      },
      {
        path: "/sparring/{session_id}/message",
        method: "POST",
        category: "Sparring Arena",
        description: "Send a message in an active sparring session.",
        requestBody: { message: "string" },
        responseSample: { botReply: "...", frameShift: 8, coachWhisper: "..." },
      },
      {
        path: "/sparring/{session_id}/verdict",
        method: "POST",
        category: "Arbitration & Ruling",
        description: "Get the referee's final verdict on a sparring session.",
        responseSample: { winner: "user", finalFrameUser: 72, finalFrameAI: 28 },
      },
      {
        path: "/war-room/stats",
        method: "GET",
        category: "Tactician Stats",
        description: "Get the current user's stats, credits, and recommended move.",
        responseSample: { handle: "...", level: 4, clean_wins: 2, credits: 20 },
      },
    ],
  };
}
...
export async function testWebhookEndpoint(params: {
  ...
  const dispatchEvent = {
    event: params.event || "case.created",
  ...
  try {
    ...
    return {
      ...
      deliveredTo: params.webhookUrl,
      eventDispatched: dispatchEvent,
    };
  } catch (err: any) {
    return {
      success: false,
      error: err?.message || "Connection refused / invalid destination / CORS blocked",
      deliveredTo: params.webhookUrl,
      eventDispatched: dispatchEvent,
    };
  }
...
export async function testRawEndpoint(path: string, method: string, body?: any): Promise<{ status: number; statusText: string; data: any; elapsedMs: number }> {
  const start = performance.now();
  ...
  return { status: res.status, statusText: res.statusText, data, elapsedMs };
}
